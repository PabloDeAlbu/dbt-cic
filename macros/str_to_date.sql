...
(CASE 
    -- Manejar el formato con '/', ej: '2018/12/05' o '2018/12'
    ...
    -- Manejar el formato 'YYYY-' o 'YYYY'
    ...
    -- Manejar el caso de 'YYYY seguido de texto', ej: '1906 TOMO II'
    WHEN ({{str_date}} ~ '^\d{4}\s.+$') THEN CONCAT(SUBSTRING({{str_date}}, 1, 4), '-01-01')

    -- Validar formato general de fecha (YYYY-MM-DD)
    WHEN ({{str_date}} ~ '^\d{4}-\d{1,2}-\d{1,2}$') 
    ...
    -- Devolver una "fecha de error" si el formato es incorrecto o tiene valores inválidos
    ELSE '9999-12-31'
END)::date
{% endmacro %}
