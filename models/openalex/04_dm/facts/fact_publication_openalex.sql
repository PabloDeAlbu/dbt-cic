...
WITH base as (
    ...
    FROM {{ref('sal_openalex_work')}} sal
    INNER JOIN {{ref('sat_openalex_work')}} sat ON sal.work_hk = sat.work_hk
    INNER JOIN {{ref('dim_pid_openalex')}} dim_pid ON dim_pid.work_hk = sal.work_hk
    INNER JOIN {{ref('link_openalex_work2type')}} link_work_type ON link_work_type.work_hk = sal.work_hk
